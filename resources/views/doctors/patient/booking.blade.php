...
<x-app-layout>
    {{-- 
        DATA PREPARATION 
        We prepare the structure so the View is clean.
    --}}
    @php
        $daysData = [];
        
        foreach ($next7Days as $day) {
            $dayName = $day->locale('id')->translatedFormat('l'); // Senin, Selasa...
            $dateString = $day->toDateString(); // 2023-10-25
            $formattedDate = $day->locale('id')->translatedFormat('d M'); // 25 Okt
            ...
            if ($schedule) {
                ...
                for ($i = 0; $i < $schedule->patient_slot; $i++) {
                    ...
                    $booked = $appointments->contains(function ($appt) use ($schedule, $day, $slotStart) {
                        return $appt->id_doctor_schedule === $schedule->id_doctor_schedule &&
                               ...
                               Carbon::parse($appt->appointment_date)->isSameDay($day) &&
                               Carbon::parse($appt->appointment_time)->equalTo($slotStart);
                    });

                    $slotDateTime = Carbon::parse($day->format('Y-m-d') . ' ' . $slotStart->format('H:i:s'));
                ...
                // Only add day if it has a schedule
                $daysData[] = [
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6 px-4 md:px-0">
                ...
                <a href="{{ route('doctors.searchPage') }}" class="text-gray-500 hover:text-gray-900 font-medium text-sm flex items-center gap-2">
                    &larr; Kembali
                </a>
            </div>
                ...
                <div class="flex-1 p-8">
                    <form id="bookingForm" action="" method="POST">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                                @forelse ($daysData as $index => $data)
                                    <button type="button" 
                                        onclick="selectDay('{{ $data['date_string'] }}', '{{ $data['schedule_id'] }}', '{{ $data['route_url'] }}', '{{ $data['doctor_name'] }}', '{{ $data['specialty'] }}', this)"
                                        ...
                                        data-date="{{ $data['date_string'] }}">
                                        <span class="text-xs font-medium uppercase tracking-wider mb-1">{{ substr($data['day_name'], 0, 3) }}</span>
                        ...
                        <div class="mb-8">
                            ...
                            {{-- We create a hidden container for EACH day's slots --}}
                            @foreach ($daysData as $index => $data)
                                <div id="slots-{{ $data['date_string'] }}" class="slot-container {{ $index === 0 ? 'grid' : 'hidden' }} grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                            ...
                            <p id="time-error" class="hidden text-red-500 text-sm mt-2 font-medium">Silakan pilih jam konsultasi.</p>
                        </div>
    ...
    {{-- JAVASCRIPT LOGIC --}}
    <script>
        // Init logic for the first available day
        document.addEventListener('DOMContentLoaded', () => {
            const firstTab = document.querySelector('.day-tab');
            if(firstTab) {
                // Manually trigger click on first tab to populate hidden fields initially
                firstTab.click();
            }
        });

        function selectDay(dateString, scheduleId, routeUrl, doctorName, specialty, element) {
            ...
            // 4. RESET: Clear time selection when changing days
            resetTimeSelection();
        }

        function selectTime(time, element) {
            // 1. VISUAL: Update Time Button Styles
