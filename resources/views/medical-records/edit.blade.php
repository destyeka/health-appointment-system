<x-app-layout>
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Edit Rekam Medis</h1>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select id="id_appointment" name="id_appointment" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-[#009688] focus:ring focus:ring-[#009688] focus:ring-opacity-50" required>
                        <option value="" disabled>Pilih janji temu yang sudah selesai...</option>
                        @foreach ($appointments as $appointment)
                            <option value="{{ $appointment->id_appointment }}" {{ old('id_appointment', $medical_record->id_appointment) == $appointment->id_appointment ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="diagnosis" name="diagnosis" value="{{ old('diagnosis', $medical_record->diagnosis) }}" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-[#009688] focus:ring focus:ring-[#009688] focus:ring-opacity-50" required>
                    @error('diagnosis')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="treatment" name="treatment" rows="4" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-[#009688] focus:ring focus:ring-[#009688] focus:ring-opacity-50" required>{{ old('treatment', $medical_record->treatment) }}</textarea>
                    @error('treatment')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-[#009688] focus:ring focus:ring-[#009688] focus:ring-opacity-50">{{ old('notes', $medical_record->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
